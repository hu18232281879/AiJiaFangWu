...
func InitDb()(*gorm.DB,error) {
	db, err := gorm.Open("mysql",  utils.MySQL_UserName+":"+utils.MySQL_Pwd+"@tcp("+utils.MySQL_Address+":"+utils.MySQL_Port+")/"+utils.MySQL_DB+"?loc=Local&parseTime=true")
	if err != nil {
		return nil,err
	}
	...
	db.AutoMigrate(new(User), new(House), new(Area), new(Facility), new(OrderHouse), new(HouseImage))
	return db,nil
